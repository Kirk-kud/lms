...
export default function ClassCalendarPageClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-4 sm:p-8">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#9C949A', margin: '4px 0 0' }}>
            Assignment due dates for this class
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button
            onClick={() => setCurrentMonth((m) => subMonths(m, 1))}
            style={{ width: '32px', height: '32px', border: '1px solid #E5E5E5', borderRadius: '7px', background: '#FFF', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B6168' }}
      ...
      {!isLoading && (
        <div style={{ border: '1px solid #ECE6E0', borderRadius: '10px', overflow: 'hidden', background: '#FFFFFF' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid #ECE6E0' }}>
            {DAY_LABELS.map((d) => (
              <div key={d} style={{ padding: '10px 0', textAlign: 'center', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9C949A', borderRight: '1px solid #ECE6E0' }}>
                {d}
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
            {days.map((day, i) => {
              ...
              return (
                <button
                  key={key}
                  onClick={() => { if (dayAssignments.length > 0) setSelectedDay(isSelected ? null : day) }}
                ...
                >
                  <div style={{ width: '24px', height: '24px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: today ? 700 : 400, color: !isCurrentMonth ? '#D0C8C6' : today ? '#FFFFFF' : '#0A0A0B', background: today ? '#8B1A2F' : 'transparent', flexShrink: 0 }}>
                    {format(day, 'd')}
                  </div>
                  {dayAssignments.slice(0, 2).map((a) => {
                    ...
                    return (
                      <div key={a.id} style={{ fontSize: '10px', fontWeight: 600, color, background: bg, borderRadius: '4px', padding: '2px 5px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>
                        {a.title}
                      </div>
                    )
                  })}
      ...
      {selectedDay && selectedAssignments.length > 0 && (
        <div style={{ marginTop: '16px', border: '1px solid #ECE6E0', borderRadius: '10px', overflow: 'hidden', background: '#FFFFFF' }}>
          ...
          {selectedAssignments.map((a, i) => {
            ...
            return (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '14px 18px', borderBottom: i < selectedAssignments.length - 1 ? '1px solid #ECE6E0' : 'none', background: 'transparent', border: 'none', cursor: 'pointer', gap: '12px', textAlign: 'left', transition: 'background 100ms ease' }}
                onMouseEnter={(e) => (e.currentTarget.style.background = '#FAF7F4')}
              ...
              >
                ...
                <span style={{ fontSize: '11px', fontWeight: 600, color: statusColor, background: statusBg, padding: '3px 9px', borderRadius: '4px', flexShrink: 0 }}>
                  {statusLabel}
                </span>
              </button>
            )
